...
class Device {
   ...
   public:
    ...
    void Launch(vector<int32_t> &arguments) {
        ...
        dpi_->WriteReg(address + ((cnt + 1) * 4), this->MemGetPhyAddr(a_));  // ptr(0)
        //cnt++;
        std::cout << "b: " << this->MemGetPhyAddr(b_);
        dpi_->WriteReg(address + ((cnt + 2) * 4), this->MemGetPhyAddr(b_));  // ptr(1)
...
int main(int argc, const char **argv) {
    ...
    uint32_t output_size = input_value.size();

    //vector<int32_t> arguments = {0, 0, 64, 0, 4096, 0, 128, 0, 128, 128, 0, 4, 16384};
    vector<int32_t> arguments = {0, 0, 66, 0, 4356, 0, 64, 0, 64, 64, 0, 32, 4096};

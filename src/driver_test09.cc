...
int main(int argc, const char **argv)
{
  ...
  a.shape[0] = 5;
  a.dtype.code = 0;
  a.dtype.bits = ELEMENT_SIZE_BITS;
  a.dtype.lanes = 1;
  a.ndim = 1;
  a.strides = nullptr;
  ...
  for (auto index = 0; index < a.shape[0]; index++)
  {
    ...
    *(((uint8_t *)(c.data)) + index * sizeof(uint8_t)) = 0;
    ...
    std::cout << "index = " << (int)index << "\ta = " << (int)val_a << "\t b = " << (int)val_b << "\t c = " << (int)val_c << "\n";
  }
  ...
  for (auto index = 0; index < a.shape[0]; index++)
  {
    uint8_t k = *(((uint8_t *)(b.data)) + index * sizeof(uint8_t));

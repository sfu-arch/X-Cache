...
//  ds.io.b := in_arbiter.io.out.bits.data("field1").data
  ...
  when(io.OutData(0).valid) {
    //    printf("[LOG] " + "[" + module_name + "] " + "[TID->%d] " + node_name + ": Output fired @ %d, Value: %d (%d + %d)\n", task_ID_R, cycleCount, FU.io.out, left_R.data, right_R.data)
    //    printf("[LOG] " + "[" + module_name + "] " + "[TID->%d] " + node_name + ": Output fired @ %d, io out0 = ${Hexadecimal(io.OutData(0).data)}\n", 0.U, cycleCount)
    //    printf("[LOG] " + "[" + module_name + "] " + "[TID->%d] " + node_name + ": Output fired @ %d, io.OutData(0): ${io.OutData(0).valid}\n", 0.U, cycleCount)
  }

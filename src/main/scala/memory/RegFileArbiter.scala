...
class RegFileArbiter(ID: Int,
  ...
  val io = IO(new Bundle {
    ...
    val RegFile = new RegFileBundle(Size)
  })
  ...
  io.RegFile.wen := WriteController.io.CacheReq.fire()
  io.RegFile.waddr := WriteController.io.CacheReq.bits.addr(wordindex + log2Ceil(Size) - 1, wordindex)
  io.RegFile.wdata := WriteController.io.CacheReq.bits.data
  io.RegFile.wmask := WriteController.io.CacheReq.bits.mask
  ...
  io.RegFile.raddr1 := ReadController.io.CacheReq.bits.addr(wordindex + log2Ceil(Size) - 1, wordindex)
  //  }

  ReadController.io.CacheResp.valid     := ReadValid
  ReadController.io.CacheResp.bits.tag  := ReadReq.tag
  ReadController.io.CacheResp.bits.data := io.RegFile.rdata1

...
trait CacheParams extends AXICoreBundle {
...
class CacheCPUIO(implicit p: Parameters) extends GenericParameterizedBundle {
...
class SimpleCache(val ID: Int = 0)(implicit p: Parameters) extends Module with CacheParams {
  ...
  io.nasti.aw.bits := NastiWriteAddressChannel(
    0.U, (Cat(rmeta.tag, idx_reg) << blen.U).asUInt(), log2Ceil(nastiXDataBits / 8).U, (dataBeats - 1).U)
...
//class Cache(val ID: Int = 0)(implicit p: Parameters) extends Module with CacheParams {

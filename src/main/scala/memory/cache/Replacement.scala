...
// credit to https://github.com/chipsalliance/rocket-chip/blob/master/src/main/scala/util/Replacement.scala
...
abstract class ReplacementPolicy {
    ...
    // def acce ss(touch_ways: Seq[Valid[UInt]]): Unit
    def state_read: UInt
...
object ReplacementPolicy {
    def fromString(s: String, n_ways: Int): ReplacementPolicy = s match {
...
//            val left_subtree_state  = state(tree_nways-3, right_nways-1) // state.extract replaced with state
//            val right_subtree_state = state(right_nways-2, 0)
//
//            if (left_nways > 1) {
//                // we are at a branching node in the tree with both left and right sub-trees, so recurse both sub-trees

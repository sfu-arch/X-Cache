...
class TypeMemDataFlow(implicit val p: Parameters) extends Module with CoreParams{
	...
	val StackFile = Module(new TypeStackFile(ID=0,Size=32,NReads=1,NWrites=1)(WControl=new WriteTypMemoryController(NumOps=1,BaseSize=2))(RControl=new ReadTypMemoryController(NumOps=1,BaseSize=2)))
...
Store.io.inData.bits.data       := 0x1eefbeefdeaddeadL.U

...
class simpleDataFlow(implicit p: Parameters) extends dataflowIO()(p){
  ...
  //entry
  val label_entry   = Module(new BasicBlockCtrl(1,0)(p))
  val label_ifthen  = Module(new BasicBlockCtrl(1,0)(p))
  val label_ifend6  = Module(new BasicBlockCtrl(2,0)(p))
  val label_ifthen2 = Module(new BasicBlockCtrl(1,0)(p))
  val label_ifthen4 = Module(new BasicBlockCtrl(1,0)(p))
  val label_ifend3  = Module(new BasicBlockCtrl(1,0)(p))
  val label_ifend5  = Module(new BasicBlockCtrl(1,0)(p))
  ...
  val m1 = Module(new CBranchNode(0)(p))

  //Connecting predicate bits
  m0.io.PredicateIN <> label_entry.io.PredicateOut
  m1.io.PredicateIN <> label_entry.io.PredicateOut

  m0.io.LeftIO  <> reg1.io.Data
  m0.io.RightIO <> reg2.io.Data
  m1.io.CmpIn   <> m0.io.OutIO

  //Connecting branch to the labels
  label_entry.io.Active(0)  := true.B
  label_ifthen.io.Active(0) <> m1.io.OutIO(0)
  label_ifend6.io.Active(0) <> m1.io.OutIO(1)
...
//  m3.io.OutIO.ready := io.resultReady
  io.result      := m0.io.OutIO.bits
  io.resultValid := m0.io.OutIO.valid
//

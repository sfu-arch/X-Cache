...
abstract class cilk_for_test04DFIO(implicit val p: Parameters) extends Module with HasAccelParams {
...
class cilk_for_test04DF(implicit p: Parameters) extends cilk_for_test04DFIO()(p) {
  ...
  //  %__begin.029 = phi i32 [ 0, %entry ], [ %inc, %pfor.inc ], !UID !14
  val phi__begin_0292 = Module(new PhiFastNode(NumInputs = 2, NumOutputs = 2, ID = 2, Res = true))

  //  detach within %syncreg, label %offload.pfor.body, label %pfor.inc, !UID !15, !BB_UID !16
  val detach_3 = Module(new Detach(ID = 3))

  //  %inc = add nuw nsw i32 %__begin.029, 1, !UID !17
  val binaryOp_inc4 = Module(new ComputeNode(NumOuts = 2, ID = 4, opCode = "add")(sign = false))

  //  %exitcond = icmp eq i32 %inc, 20, !UID !18
  ...
  //  call void @cilk_for_test04_detach1(i32* %a, i32 %__begin.029, i32* %b, i32* %c), !UID !25
  ...
  //i32 20
  val const2 = Module(new ConstFastNode(value = 20, ID = 2))
  ...
  phi__begin_0292.io.InData(1) <> Loop_0.io.CarryDepenOut.elements("field0")(0)
  ...
  phi__begin_0292.io.enable <> bb_pfor_detach2.io.Out(1)
  ...
  phi__begin_0292.io.Mask <> bb_pfor_detach2.io.MaskBB(0)
  ...
  phi__begin_0292.io.InData(0) <> const0.io.Out
  ...
  binaryOp_inc4.io.LeftIO <> phi__begin_0292.io.Out(0)

  call_8_out.io.In.elements("field1") <> phi__begin_0292.io.Out(1)
...
object cilk_for_test04Top extends App {
  val dir = new File("RTL/cilk_for_test04Top");
  dir.mkdirs

...
abstract class cilk_for_test05_optDFIO(implicit val p: Parameters) extends Module with HasAccelParams {
...
class cilk_for_test05_optDF(implicit p: Parameters) extends cilk_for_test05_optDFIO()(p) {
  ...
  val bb_entry0 = Module(new BasicBlockNoMaskFastNode(NumOuts = 1, BID = 0))

  val bb_pfor_cond1 = Module(new LoopHead(NumOuts = 5, NumPhi = 1, BID = 1))

  val bb_pfor_detach2 = Module(new BasicBlockNoMaskFastNode(NumOuts = 1, BID = 2))

  val bb_pfor_inc3 = Module(new BasicBlockNoMaskFastNode(NumOuts = 3, BID = 3))

  val bb_pfor_end4 = Module(new BasicBlockNoMaskFastNode(NumOuts = 1, BID = 4))
  ...
  val icmp_cmp2 = Module(new ComputeNode(NumOuts = 1, ID = 2, opCode = "lt")(sign = false))

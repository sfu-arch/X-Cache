...
abstract class cilk_for_test02DFIO(implicit val p: Parameters) extends Module with HasAccelParams {
...
class cilk_for_test02DF(implicit p: Parameters) extends cilk_for_test02DFIO()(p) {
  ...
  val icmp_exitcond418 = Module(new ComputeNode(NumOuts = 1, ID = 8, opCode = "eq")(sign = false))

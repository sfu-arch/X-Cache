...
object Data_vector_scale_FlowParam{
  ...
  //  %i.0 = phi i32 [ 0, %entry ], [ %inc, %pfor.inc ], !UID !12, !ScalaLabel !13
  val phi1_in = Map(
    "add5" -> 0
  )
  ...
  //  br i1 %cmp, label %pfor.detach, label %pfor.end, !UID !16, !BB_UID !17, !ScalaLabel !18
  val br3_in = Map(
    "icmp2" -> 0
  )


  //  detach label %offload.pfor.body, label %pfor.inc, !UID !19, !BB_UID !20, !ScalaLabel !21
  val detach4_in = Map(
    "" -> 0,
    "" -> 1
  )
  ...
  //  %inc = add nsw i32 %i.0, 1, !UID !22, !ScalaLabel !23
  val add5_in = Map(
    "phi1" -> 1
  )


  //  sync label %pfor.end.continue, !UID !36, !BB_UID !37, !ScalaLabel !38
  val sync7_in = Map(
    "" -> 2
  )


  //  ret void, !UID !39, !BB_UID !40, !ScalaLabel !41
  val ret8_in = Map(

  )


  //  call void @vector_scale_detach1(i32* %a, i32 %i.0, i32* %c, i32 %scale), !UID !42, !ScalaLabel !43
  val call9_in = Map(
    "field0" -> 0,
    "phi1" -> 2,
    "field1" -> 0,
  ...
  //  reattach label %pfor.inc, !UID !44, !BB_UID !45, !ScalaLabel !46
...
*/
  ...
  val bb_pfor_end = Module(new BasicBlockNoMaskNode(NumInputs = 1, NumOuts = 5, BID = 4))
  ...
  //  br label %pfor.cond, !UID !9, !BB_UID !10, !ScalaLabel !11
  ...
  //  br label %pfor.cond, !llvm.loop !24, !UID !33, !BB_UID !34, !ScalaLabel !35
  ...
  /**
    ...
    */
  val param = Data_vector_scale_FlowParam
  ...
  bb_pfor_end_continue.io.predicateIn <> sync7.io.Out(0) // Manually added
  ...
  loop_L_0_liveIN_3.io.enable <> bb_pfor_end.io.Out(4)
  ...
  icmp2.io.RightIO <> loop_L_0_liveIN_3.io.Out(param.icmp2_in("field3"))
  ...
  call9.io.In.data("field0") <> loop_L_0_liveIN_0.io.Out(param.call9_in("field0"))

  // Wiring Call instruction to the loop header
  call9.io.In.data("field1") <> phi1.io.Out(param.call9_in("phi1"))

  // Wiring Call instruction to the loop header
  call9.io.In.data("field2") <> loop_L_0_liveIN_1.io.Out(param.call9_in("field1"))

  // Wiring Call instruction to the loop header
  call9.io.In.data("field3") <> loop_L_0_liveIN_2.io.Out(param.call9_in("field2"))



}

class vector_scaleTopIO(implicit val p: Parameters)  extends Module with CoreParams with CacheParams {
...
class vector_scaleTop()(implicit p: Parameters) extends vector_scaleTopIO  {
  ...
  for (i <- 0 until children) {
    CacheArbiter.io.cpu.CacheReq(i) <> vector_scale_detach(i).io.CacheReq
    vector_scale_detach(i).io.CacheResp <> CacheArbiter.io.cpu.CacheResp(i)
  }
  ...
  TaskControllerModule.io.parentIn(0) <> vector_scale.io.call9_out
  ...
  vector_scale.io.call9_in <> TaskControllerModule.io.parentOut(0)

...
abstract class cilk_saxpyDFIO(implicit val p: Parameters) extends Module with HasAccelParams {
...
class cilk_saxpyDF(implicit p: Parameters) extends cilk_saxpyDFIO()(p) {
  ...
  val icmp_cmp170 = Module(new ComputeNode(NumOuts = 1, ID = 0, opCode = "gt")(sign = false))

...
abstract class cilk_for_test05_detach1DFIO(implicit val p: Parameters) extends Module with CoreParams {
...
class cilk_for_test05_detach1DF(implicit p: Parameters) extends cilk_for_test05_detach1DFIO()(p) {
  ...
  val bb_my_pfor_body0 = Module(new BasicBlockNoMaskNode(NumInputs = 1, NumOuts = 6, BID = 0))

  val bb_my_if_then1 = Module(new BasicBlockNoMaskNode(NumInputs = 1, NumOuts = 8, BID = 1))

  val bb_my_if_end2 = Module(new BasicBlockNoMaskNode(NumInputs = 2, NumOuts = 1, BID = 2))

  val bb_my_pfor_preattach3 = Module(new BasicBlockNoMaskNode(NumInputs = 1, NumOuts = 1, BID = 3))
  ...
  bb_my_if_end2.io.predicateIn <> br_13.io.Out(0)

  bb_my_if_end2.io.predicateIn <> br_23.io.Out(0)
  ...
  ret_15.io.enable <> bb_my_pfor_preattach3.io.Out(0)
  ...
  ret_15.io.In.elements("field0") <> st_22.io.Out(0)
...
object cilk_for_test05_detach1Main extends App {
  val dir = new File("RTL/cilk_for_test05_detach1") ; dir.mkdirs

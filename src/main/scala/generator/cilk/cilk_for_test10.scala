...
object Data_cilk_for_test10_FlowParam{
  ...
  //  %call = call i32 @cilk_for_test10_inner(i32 %n), !UID !24, !ScalaLabel !25
  val call7_in = Map(
    "field1" -> 0,
    "" -> 2
  )
...
abstract class cilk_for_test10DFIO(implicit val p: Parameters) extends Module with CoreParams {
...
class cilk_for_test10DF(implicit p: Parameters) extends cilk_for_test10DFIO()(p) {
  ...
  /**
    ...
    */
  val param = Data_cilk_for_test10_FlowParam
...
//  reattach12.io.enable <> bb_pfor_preattach.io.Out(param.bb_pfor_preattach_activate("reattach12"))
  ...
  store1.io.memResp  <> RegisterFile.io.WriteOut(0)
  RegisterFile.io.WriteIn(0) <> store1.io.memReq
...
object cilk_for_test10Main extends App {
  val dir = new File("RTL/cilk_for_test10") ; dir.mkdirs
  implicit val p = config.Parameters.root((new MiniConfig).toInstance)
  val chirrtl = firrtl.Parser.parse(chisel3.Driver.emit(() => new cilk_for_test10DF()))

...
object Data_cilk_for_test09_inner_FlowParam{
  ...
  //  %a.0 = phi i32 [ 0, %entry ], [ %inc, %for.inc ], !UID !10, !ScalaLabel !11
  val phi1_in = Map(
    "add5" -> 0
  )


  //  %j.0 = phi i32 [ 0, %entry ], [ %inc1, %for.inc ], !UID !12, !ScalaLabel !13
  val phi2_in = Map(
    "add7" -> 0
  )


  //  %cmp = icmp slt i32 %j.0, %n, !UID !14, !ScalaLabel !15
  val icmp3_in = Map(
    "phi2" -> 0,
    "field0" -> 0
  )
  ...
  //  %inc = add nsw i32 %a.0, 1, !UID !19, !ScalaLabel !20
  val add5_in = Map(
    "phi1" -> 0
  )


  //  %inc1 = add nsw i32 %j.0, 1, !UID !24, !ScalaLabel !25
  val add7_in = Map(
    "phi2" -> 1
  )

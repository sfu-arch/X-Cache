...
object Data_cilk_for_test11_FlowParam{
  ...
  //  %cmp17 = icmp sgt i32 %n, 0, !UID !2, !ScalaLabel !3
  val icmp0_in = Map(
    "field1" -> 0
  )


  //  br i1 %cmp17, label %pfor.detach.preheader, label %pfor.cond.cleanup, !UID !4, !BB_UID !5, !ScalaLabel !6
  val br1_in = Map(
    "icmp0" -> 0
  )


  //  sync label %pfor.end.continue, !UID !13, !BB_UID !14, !ScalaLabel !15
  val sync4_in = Map(
    "" -> 0
  )


  //  ret i32 1, !UID !16, !BB_UID !17, !ScalaLabel !18
  val ret5_in = Map(

  )


  //  %i.018 = phi i32 [ %inc, %pfor.inc ], [ 0, %pfor.detach.preheader ], !UID !19, !ScalaLabel !20
  val phi6_in = Map(
    "add8" -> 0
  )
  ...
  //  %inc = add nuw nsw i32 %i.018, 1, !UID !24, !ScalaLabel !25
  val add8_in = Map(
    "phi6" -> 0
  )


  //  %exitcond = icmp eq i32 %inc, %n, !UID !26, !ScalaLabel !27
  val icmp9_in = Map(
    "add8" -> 1,
    "field1" -> 1
  )
  ...
  //  br i1 %exitcond, label %pfor.cond.cleanup.loopexit, label %pfor.detach, !llvm.loop !28, !UID !30, !BB_UID !31, !ScalaLabel !32
  val br10_in = Map(
    "icmp9" -> 0
  )


  //  call void @cilk_for_test11_detach1(%struct.Edge* %E, i32 %i.018), !UID !33, !ScalaLabel !34
  val call11_in = Map(
    "field0" -> 0,
    "phi6" -> 1,
    "" -> 3
...
abstract class cilk_for_test11DFIO(implicit val p: Parameters) extends Module with CoreParams {
  val io = IO(new Bundle {
    val in = Flipped(Decoupled(new Call(List(32,32))))
    val call11_out = Decoupled(new Call(List(32,32)))
...
class cilk_for_test11DF(implicit p: Parameters) extends cilk_for_test11DFIO()(p) {
  ...
  val field1_expand = Module(new ExpandNode(NumOuts=2,ID=102)(new DataBundle))
  ...
  val bb_pfor_cond_cleanup_loopexit = Module(new BasicBlockNoMaskNode(NumInputs = 1, NumOuts = 3, BID = 2))
  ...
  val bb_pfor_detach = Module(new  LoopHead(NumOuts = 2, NumPhi = 1, BID = 5))
  ...
  //  br label %pfor.cond.cleanup, !UID !10, !BB_UID !11, !ScalaLabel !12
  ...
  //  call void @cilk_for_test11_detach1(%struct.Edge* %E, i32 %i.018), !UID !33, !ScalaLabel !34
  val call11 = Module(new CallNode(ID=11,argTypes=List(32,32),retTypes=List(32)))
  ...
  /**
    ...
    */
  val param = Data_cilk_for_test11_FlowParam
  ...
  bb_pfor_detach.io.activate <> br2.io.Out(param.br2_brn_bb("bb_pfor_detach"))
  ...
  bb_pfor_detach.io.loopBack <> br10.io.Out(param.br10_brn_bb("bb_pfor_detach"))
...
//  reattach12.io.enable <> bb_offload_pfor_body.io.Out(param.bb_offload_pfor_body_activate("reattach12"))
  ...
  icmp0.io.LeftIO <> field1_expand.io.Out(1)
  ...
  icmp9.io.RightIO <> loop_L_0_liveIN_1.io.Out(0)  // manual
  ...
  // Wiring Call instruction to the loop header
  call11.io.In.data("field1") <> phi6.io.Out(param.call11_in("phi6"))
...
object cilk_for_test11Main extends App {
  val dir = new File("RTL/cilk_for_test11") ; dir.mkdirs
  implicit val p = config.Parameters.root((new MiniConfig).toInstance)
  val chirrtl = firrtl.Parser.parse(chisel3.Driver.emit(() => new cilk_for_test11DF()))

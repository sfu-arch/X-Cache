...
abstract class test20DFIO(implicit val p: Parameters) extends Module with CoreParams {
...
class test20DF(implicit p: Parameters) extends test20DFIO()(p) {
  ...
  val bb_for_cond_cleanup4 = Module(new BasicBlockNoMaskNode(NumInputs = 2, NumOuts = 2, BID = 4))
  ...
  val br_6 = Module(new UBranchFastNode(ID = 6))
  ...
  bb_for_body4_lr_ph2.io.activate <> Loop_1.io.activate

  bb_for_body4_lr_ph2.io.loopBack <> br_10.io.FalseOutput(0)

  bb_for_cond_cleanup_loopexit3.io.predicateIn <> Loop_1.io.endEnable

  bb_for_cond_cleanup4.io.predicateIn <> br_1.io.TrueOutput(0)

  bb_for_cond_cleanup4.io.predicateIn <> br_6.io.Out(0)

  bb_for_cond_cleanup35.io.predicateIn <> Loop_0.io.endEnable

  bb_for_body46.io.activate <> Loop_0.io.activate

  bb_for_body46.io.loopBack <> br_19.io.FalseOutput(0)
  ...
  Loop_0.io.latchEnable <> br_19.io.FalseOutput(1)

  Loop_0.io.loopExit(0) <> br_19.io.TrueOutput(0)

  Loop_1.io.enable <> br_2.io.Out(0)

  Loop_1.io.latchEnable <> br_10.io.FalseOutput(1)

  Loop_1.io.loopExit(0) <> br_10.io.TrueOutput(0)
  ...
  const1.io.enable <> bb_for_body4_lr_ph2.io.Out(0)

  phi_j_0253.io.enable <> bb_for_body4_lr_ph2.io.Out(1)

  binaryOp_mul4.io.enable <> bb_for_body4_lr_ph2.io.Out(2)
  ...
  const2.io.enable <> bb_for_cond_cleanup4.io.Out(0)

  ret_7.io.In.enable <> bb_for_cond_cleanup4.io.Out(1)
  ...
  phi_k_02311.io.enable <> bb_for_body46.io.Out(3)
  ...
  phi_j_0253.io.Mask <> bb_for_body4_lr_ph2.io.MaskBB(0)

  phi_k_02311.io.Mask <> bb_for_body46.io.MaskBB(0)
  ...
  phi_j_0253.io.InData(0) <> const1.io.Out

  ret_7.io.In.data("field0") <> const2.io.Out

  binaryOp_inc88.io.RightIO <> const3.io.Out

  phi_k_02311.io.InData(0) <> const4.io.Out
  ...
  binaryOp_mul4.io.LeftIO <> phi_j_0253.io.Out(0)

  binaryOp_inc88.io.LeftIO <> phi_j_0253.io.Out(1)

  phi_j_0253.io.InData(1) <> binaryOp_inc88.io.Out(0)

  icmp_exitcond269.io.LeftIO <> binaryOp_inc88.io.Out(1)

  br_10.io.CmpIO <> icmp_exitcond269.io.Out(0)

  binaryOp_add12.io.LeftIO <> phi_k_02311.io.Out(0)

  binaryOp_inc17.io.LeftIO <> phi_k_02311.io.Out(1)
  ...
  phi_k_02311.io.InData(1) <> binaryOp_inc17.io.Out(0)
...
object test20Main extends App {
  val dir = new File("RTL/test20") ; dir.mkdirs
  implicit val p = config.Parameters.root((new MiniConfig).toInstance)
  val chirrtl = firrtl.Parser.parse(chisel3.Driver.emit(() => new test20DF()))

...
class test09DF(implicit p: Parameters) extends test09DFIO()(p) {
  ...
  val foo_0 = Module(new PhiNode(NumInputs = 2, NumOuts = 1, ID = 1))

  //  %i.0 = phi i32 [ 0, %entry ], [ %inc, %for.inc ]
  val i_0 = Module(new PhiNode(NumInputs = 2, NumOuts = 3, ID = 2))
  ...
  Loop_0.io.liveOut(0) <> foo_0.io.Out(0)
  ...
  add.io.enable <> for_body.io.Out(1)
  ...
  add.io.RightIO <> const2.io.Out(0)

  inc.io.RightIO <> const3.io.Out(0)

  cmp.io.LeftIO <> i_0.io.Out(0)

  add.io.LeftIO <> i_0.io.Out(1)

  inc.io.LeftIO <> i_0.io.Out(2)

  br_4.io.CmpIO <> cmp.io.Out(0)

  foo_0.io.InData(1) <> add.io.Out(0)

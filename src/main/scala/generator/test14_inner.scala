...
object Data_test14_inner_FlowParam{
  ...
  //  %a.0 = phi i32 [ 0, %entry ], [ %inc, %for.inc ], !UID !11, !ScalaLabel !12
  val phi1_in = Map(
    "add5" -> 0
  )


  //  %j.0 = phi i32 [ 0, %entry ], [ %inc1, %for.inc ], !UID !13, !ScalaLabel !14
  val phi2_in = Map(
    "add7" -> 0
  )


  //  %cmp = icmp slt i32 %j.0, %n, !UID !15, !ScalaLabel !16
  val icmp3_in = Map(
    "phi2" -> 0,
    "field0" -> 0
  )
  ...
  //  %inc = add nsw i32 %a.0, 1, !UID !20, !ScalaLabel !21
  val add5_in = Map(
    "phi1" -> 0
  )


  //  %inc1 = add nsw i32 %j.0, 1, !UID !25, !ScalaLabel !26
  val add7_in = Map(
    "phi2" -> 1
  )


  //  ret i32 %a.0, !UID !38, !BB_UID !39, !ScalaLabel !40
  val ret9_in = Map(
    "phi1" -> 1
  )
...
abstract class test14_innerDFIO(implicit val p: Parameters) extends Module with CoreParams {
...
class test14_innerDF(implicit p: Parameters) extends test14_innerDFIO()(p) {
  ...
  /**
    ...
    */
  val param = Data_test14_inner_FlowParam
  ...
  loop_L_0_LiveOut_0.io.enable <> bb_for_end.io.Out(2)
...
object test14_innerMain extends App {
  val dir = new File("RTL/test14_inner") ; dir.mkdirs
  implicit val p = config.Parameters.root((new MiniConfig).toInstance)
  val chirrtl = firrtl.Parser.parse(chisel3.Driver.emit(() => new test14_innerDF()))

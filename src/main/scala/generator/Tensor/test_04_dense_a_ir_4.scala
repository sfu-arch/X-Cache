...
abstract class test_04_dense_a_ir_4DFIO(implicit val p: Parameters) extends Module with CoreParams {
...
abstract class dense04aTopIO(implicit val p: Parameters) extends Module with CoreParams {
  val io = IO(new Bundle {
    val in = Flipped(Decoupled(new Call(List(32, 32, 32, 32, 32))))
    val out = Decoupled(new Call(List()))
  })

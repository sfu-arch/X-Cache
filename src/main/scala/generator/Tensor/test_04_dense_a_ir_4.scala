...
abstract class test_04_dense_a_ir_4DFIO(implicit val p: Parameters) extends Module with HasAccelParams {
...
class test_04_dense_a_ir_4DF(implicit p: Parameters) extends test_04_dense_a_ir_4DFIO()(p) {
  ...
  //  %1 = bitcast i8* %arg1.untyped to [1 x [8 x float]]*, !UID !5
  ...
  //  %3 = bitcast i8* %arg2.untyped to [8 x float]*, !UID !10
  ...
  //  %dot = bitcast i8* %5 to [1 x [8 x float]]*, !UID !13
  ...
  //  %tmp = getelementptr [1 x [8 x float]], [1 x [8 x float]]* %1, i64 0, i64 0, !UID !17
  val Gep_tmp12 = Module(new GepNode(NumIns = 2, NumOuts = 1, ID = 12)(ElementSize = 32, ArraySize = List(32)))

  //  %tmp1 = getelementptr [8 x float], [8 x float]* %tmp, i64 0, i64 %dot.indvar.rhs.16, !UID !18
  val Gep_tmp113 = Module(new GepNode(NumIns = 2, NumOuts = 1, ID = 13)(ElementSize = 4, ArraySize = List(32)))
  ...
  //val FP_17 = Module(new FPComputeNode(NumOuts = 1, ID = 17, opCode = "fmul")(t = fType))
  val FP_17 = Module(new FPCustomMultiplierNode(NumOuts = 1, ID = 15, opCode = "fmul")(t = fType))
  ...
  //  %tmp3 = getelementptr [8 x float], [8 x float]* %tmp2, i64 0, i64 %dot.indvar.rhs.16, !UID !25
  val Gep_tmp320 = Module(new GepNode(NumIns = 2, NumOuts = 1, ID = 20)(ElementSize = 4, ArraySize = List(32)))
  ...
  //  %fusion = bitcast i8* %9 to [8 x float]*, !UID !32
  ...
  //  %10 = getelementptr inbounds [8 x float], [8 x float]* %3, i64 0, i64 %fusion.indvar.dim.02, !UID !36
  val Gep_29 = Module(new GepNode(NumIns = 2, NumOuts = 1, ID = 29)(ElementSize = 4, ArraySize = List(32)))

  //  %11 = load float, float* %10, align 4, !UID !37
  val ld_30 = Module(new UnTypLoad(NumPredOps = 0, NumSuccOps = 0, NumOuts = 1, ID = 30, RouteID = 7))

  //  %tmp4 = getelementptr [1 x [8 x float]], [1 x [8 x float]]* %dot, i64 0, i64 0, !UID !38
  val Gep_tmp431 = Module(new GepNode(NumIns = 2, NumOuts = 1, ID = 31)(ElementSize = 32, ArraySize = List(32)))

  //  %tmp5 = getelementptr [8 x float], [8 x float]* %tmp4, i64 0, i64 %fusion.indvar.dim.02, !UID !39
  val Gep_tmp532 = Module(new GepNode(NumIns = 2, NumOuts = 1, ID = 32)(ElementSize = 4, ArraySize = List(32)))
  ...
  //i64 7
  val const11 = Module(new ConstFastNode(value = 7, ID = 11))
  ...
  //i64 7
  val const19 = Module(new ConstFastNode(value = 7, ID = 19))
...
abstract class dense04aTopIO(implicit val p: Parameters) extends Module with HasAccelParams {
  val io = IO(new Bundle {
    val in = Flipped(Decoupled(new Call(List(32, 32, 32, 32, 32))))
    val out = Decoupled(new Call(List()))
  })
...
class dense04aMain(implicit p: Parameters) extends dense04aTopIO {

  // Wire up the cache and modules under test.
  val test = Module(new test_04_dense_a_ir_4DF())

...
abstract class test_07_softmax_b_ir_4DFIO(implicit val p: Parameters) extends Module with HasAccelParams {
...
abstract class softmax07bTopIO(implicit val p: Parameters) extends Module with HasAccelParams {
  val io = IO(new Bundle {
    val in = Flipped(Decoupled(new Call(List(32, 32, 32, 32, 32))))
    val out = Decoupled(new Call(List()))
  })

...
class Dyser1(SelN: Int, EnN: Bool,
             SelE: Int, EnE: Bool,
             SelW: Int, EnW: Bool,
             SelS: Int, EnS: Bool,
             SelNE: Int, EnNE: Bool,
             ...
             SelSE: Int, EnSE: Bool,
             SelSW: Int, EnSW: Bool
            ) (implicit p: Parameters) extends DyserIO()(p) {
  val param = switchParam1
  //  val param = conf
  //Five 4:1 Muxes
  val DMuxN = Module(new DyserMux(NInputs = param.numInputs("numInN"), Sel = SelN, En = EnN)(p))
  val DMuxE = Module(new DyserMux(NInputs = param.numInputs("numInE"), Sel = SelE, En = EnE)(p))
  val DMuxW = Module(new DyserMux(NInputs = param.numInputs("numInW"), Sel = SelW, En = EnW)(p))
  val DMuxS = Module(new DyserMux(NInputs = param.numInputs("numInS"), Sel = SelS, En = EnS)(p))
  val DMuxNE = Module(new DyserMux(NInputs= param.numInputs("numInNE"), Sel =SelNE, En= EnNE)(p))

  //Three 5:1 Muxes

  val DMuxNW = Module(new DyserMux(NInputs =param.numInputs("numInNW") , Sel = SelNW, En = EnNW)(p))
  val DMuxSE = Module(new DyserMux(NInputs =param.numInputs("numInSE") , Sel = SelSE, En = EnSE)(p))
  val DMuxSW = Module(new DyserMux(NInputs =param.numInputs("numInSW") , Sel = SelSW, En = EnSW)(p))

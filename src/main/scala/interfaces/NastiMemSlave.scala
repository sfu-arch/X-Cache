...
class InitParams(implicit p: Parameters) extends AccelBundle( )(p) with CacheParams {
  val addr = UInt(nastiXAddrBits.W)
  val data = UInt(nastiXDataBits.W)
}

class NastiMemSlaveIO(implicit p: Parameters) extends AccelBundle( )(p) with CacheParams {

...
class LoadNodeTests1(c: MemDataFlow) extends PeekPokeTester(c) {
  for (t <- 0 until 12) {

    //IF ready is set
    // send address
    if(t>0) poke(c.io.Out1.ready, true)
    if(t>2) poke(c.io.Out2.ready, true)
...
class LoadNodeTester extends ChiselFlatSpec {
  ...
  backends foreach {backend =>
    it should s"correctly find decoupled behaviour -  $backend" in {
      Driver(() => new MemDataFlow(32), backend)((c) => new LoadNodeTests1(c)) should be (true)
    }
  }
}

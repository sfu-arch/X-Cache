...
class cilk_for_test07MainDirect(implicit p: Parameters) extends cilk_for_test07MainIO {
  ...
  cache.io.cpu.req <> cilk_for_test07_detach.io.CacheReq
  cilk_for_test07_detach.io.CacheResp <> cache.io.cpu.resp
...
class cilk_for_test07MainTM(implicit p: Parameters) extends cilk_for_test07MainIO  {
  ...
  for (i <- 0 until children) {
    CacheArbiter.io.cpu.CacheReq(i) <> cilk_for_test07_detach(i).io.CacheReq
    cilk_for_test07_detach(i).io.CacheResp <> CacheArbiter.io.cpu.CacheResp(i)
  }
  ...
  for (i <- 0 until children ) {
    cilk_for_test07_detach(i).io.in <> TaskControllerModule.io.childOut(i)
...
class cilk_for_test07Test01[T <: cilk_for_test07MainIO](c: T) extends PeekPokeTester(c) {
  ...
  val outAddrVec = List.range(256, 256+36, 4)
  val outDataVec = List(2,8,18,32,50,72,98,128,0)
  ...
  poke(c.io.in.bits.data("field1").data, 72.U)   // Array b[] base address
  poke(c.io.in.bits.data("field1").predicate, true.B)
  poke(c.io.in.bits.data("field2").data, 256.U)   // Array c[] base address

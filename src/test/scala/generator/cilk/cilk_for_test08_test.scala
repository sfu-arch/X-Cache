...
class cilk_for_test08MainDirect(implicit p: Parameters) extends cilk_for_test08MainIO {
  ...
  cache.io.cpu.req <> cilk_for_test08_detach.io.MemReq
  cilk_for_test08_detach.io.MemResp <> cache.io.cpu.resp
...
class cilk_for_test08MainTM(implicit p: Parameters) extends cilk_for_test08MainIO  {
...
class cilk_for_test08Test01[T <: cilk_for_test08MainIO](c: T) extends PeekPokeTester(c) {
  val pixels = 8
...
//  poke(c.io.in.bits.data("field2").predicate, false.B)
  ...
  poke(c.io.in.bits.data("field0").data, 0.U)     // Array rgb[] base address
...
class cilk_for_test08Tester1 extends FlatSpec with Matchers {
  implicit val p = config.Parameters.root((new MiniConfig).toInstance)
  it should "Check that cilk_for_test08 works correctly." in {
    // iotester flags:
    ...
    chisel3.iotesters.Driver.execute(
     Array(
       ...
       "-td", "test_run_dir",
       "-tts", "0001"),
     () => new cilk_for_test08MainDirect()) {
     c => new cilk_for_test08Test01(c)
    } should be(true)
  }
}

class cilk_for_test08Tester2 extends FlatSpec with Matchers {
  implicit val p = config.Parameters.root((new MiniConfig).toInstance)
  // iotester flags:
  // -ll  = log level <Error|Warn|Info|Debug|Trace>
  // -tbn = backend <firrtl|verilator|vcs>

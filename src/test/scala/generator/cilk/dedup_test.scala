...
class dedupMainDirect(implicit p: Parameters) extends dedupMainIO {
  ...
  val dedup_S2 = Module(new dedup_S2DF())
  val dedup_S3 = Module(new dedup_S3DF())
  val dedup_S4 = Module(new dedup_S4DF())
  ...
  io.out <> dedup.io.out
  dedup_S4.io.in <> dedup.io.call_1_out      // dedup->S4
  dedup.io.call_1_in <> dedup_S4.io.out
  dedup_S2.io.in <> dedup.io.call_15_out     // dedup->S2
  dedup.io.call_15_in <> dedup_S2.io.out
  dedup_S3.io.in <> dedup_S2.io.call_8_out  // S2->S3
  dedup_S2.io.call_8_in <> dedup_S3.io.out
...
class dedupTest01[T <: dedupMainIO](c: T) extends PeekPokeTester(c) {
  ...
  val queueAddrVec = List.range(queueBase, queueBase+4*10,4)

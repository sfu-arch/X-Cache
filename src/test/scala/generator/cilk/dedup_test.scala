...
class dedupMainIO(implicit val p: Parameters) extends Module with HasAccelParams
  with HasAccelShellParams
  with HasCacheAccelParams {
  val io = IO(new AccelBundle {
    val in = Flipped(Decoupled(new Call(List(32, 32, 32))))
    val addr = Input(UInt(nastiParams.addrBits.W)) // Initialization address
    val din = Input(UInt(nastiParams.dataBits.W)) // Initialization data
    val write = Input(Bool()) // Initialization write strobe
    val dout = Output(UInt(nastiParams.dataBits.W))

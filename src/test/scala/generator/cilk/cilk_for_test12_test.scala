...
class cilk_for_test12Main(implicit p: Parameters) extends cilk_for_test12MainIO {
  ...
  val MemArbiter = Module(new MemArbiter(children1+children2+children3+1))
  for (i <- 0 until children1) {
    MemArbiter.io.cpu.MemReq(i) <> cilk_for_test12_detach1(i).io.MemReq
    cilk_for_test12_detach1(i).io.MemResp <> MemArbiter.io.cpu.MemResp(i)
  }
  for (i <- 0 until children2) {
    MemArbiter.io.cpu.MemReq(children1+i) <> cilk_for_test12_detach2(i).io.MemReq
    cilk_for_test12_detach2(i).io.MemResp <> MemArbiter.io.cpu.MemResp(children1+i)
  }
  for (i <- 0 until children3) {
    MemArbiter.io.cpu.MemReq(children2+children1+i) <> cilk_for_test12_detach3(i).io.MemReq
    cilk_for_test12_detach3(i).io.MemResp <> MemArbiter.io.cpu.MemResp(children2+children1+i)
  }
  MemArbiter.io.cpu.MemReq(children1+children2+children3) <> cilk_for_test12.io.MemReq
  cilk_for_test12.io.MemResp <> MemArbiter.io.cpu.MemResp(children1+children2+children3)

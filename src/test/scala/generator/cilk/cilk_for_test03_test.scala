...
class cilk_for_test03Test01[T <: cilk_for_test03MainIO](c: T, n: Int, tiles: Int) extends PeekPokeTester(c) {
  ...
  val outAddrVec = List.range(40, 40 + (4 * 5), 4)
  ...
  poke(c.io.in.bits.data("field2").data, 40) // Array c[] base address
  ...
  it should "Check that cilk_for_test03 works correctly." in {
    ...
    chisel3.iotesters.Driver.execute(
      Array(
        ...
        "-tts", "0001"),
      () => new cilk_for_test03Main1(tiles = 1)(p.alterPartial({ case TLEN => 6 }))) {
      c => new cilk_for_test03Test01(c, 5, 1)
    } should be(true)
  }
}

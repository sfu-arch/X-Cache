...
class vector_scaleMainDirect(implicit p: Parameters) extends vector_scaleMainIO {
  ...
  cache.io.cpu.req <> vector_scale_detach.io.CacheReq
  vector_scale_detach.io.CacheResp <> cache.io.cpu.resp
  vector_scale.io.in <> io.in
  vector_scale_detach.io.in <> vector_scale.io.call9_out
  vector_scale.io.call9_in <> vector_scale_detach.io.out
...
class vector_scaleTest01[T <: vector_scaleMainIO](c: T) extends PeekPokeTester(c) {
  ...
  poke(c.io.in.bits.data("field2").data, 800.U)   // Array c[] base address

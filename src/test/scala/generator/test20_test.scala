...
class test20Main(implicit p: Parameters) extends test20MainIO {
  ...
  val test20 = Module(new test20DF())

  //Put an arbiter infront of cache
  val CacheArbiter = Module(new MemArbiter(2))

  // Connect input signals to cache
  CacheArbiter.io.cpu.MemReq(0) <> test20.io.MemReq
  test20.io.MemResp <> CacheArbiter.io.cpu.MemResp(0)
...
class test20Test01[T <: test20MainIO](c: T) extends PeekPokeTester(c) {
...
class test20Tester1 extends FlatSpec with Matchers {
  implicit val p = config.Parameters.root((new MiniConfig).toInstance)
  it should "Check that test20 works correctly." in {
    // iotester flags:
    ...
    chisel3.iotesters.Driver.execute(
      Array(
        ...
        "-tn", "test20Main",
        "-tbn", "verilator",
        "-td", "test_run_dir/test20/",
        "-tts", "0001"),
      () => new test20Main()) {
      c => new test20Test01(c)
    } should be(true)
  }
}

...
class kmpMainIO(implicit val p: Parameters)  extends Module with CoreParams with CacheParams {
...
class kmpMain(implicit p: Parameters) extends kmpMainIO {
  ...
  val kmp = Module(new kmpDF())

  //Put an arbiter infront of cache
  val CacheArbiter = Module(new MemArbiter(2))

  // Connect input signals to cache
  CacheArbiter.io.cpu.MemReq(0) <> kmp.io.MemReq
  kmp.io.MemResp <> CacheArbiter.io.cpu.MemResp(0)
...
class kmpTest01[T <: kmpMainIO](c: T) extends PeekPokeTester(c) {
...
class kmpTester1 extends FlatSpec with Matchers {
  implicit val p = config.Parameters.root((new MiniConfig).toInstance)
  it should "Check that kmp works correctly." in {
    // iotester flags:

...
class gemmMainIO(implicit val p: Parameters)  extends Module with CoreParams with CacheParams {
...
class gemmMain(implicit p: Parameters) extends gemmMainIO {
  ...
  val gemm = Module(new bbgemmDF())

  //Put an arbiter infront of cache
  val CacheArbiter = Module(new MemArbiter(2))

  // Connect input signals to cache
  CacheArbiter.io.cpu.MemReq(0) <> gemm.io.MemReq
  gemm.io.MemResp <> CacheArbiter.io.cpu.MemResp(0)
...
class gemmTest01[T <: gemmMainIO](c: T) extends PeekPokeTester(c) {
  ...
  poke(c.io.in.bits.data("field1").data, 1000)   // n
  poke(c.io.in.bits.data("field1").predicate, true.B)
  poke(c.io.in.bits.data("field2").data, 2000)   // n
...
class gemmTester1 extends FlatSpec with Matchers {
  implicit val p = config.Parameters.root((new MiniConfig).toInstance)
  it should "Check that gemm works correctly." in {
    // iotester flags:

...
import config._
import dataflow.spmvDF
import interfaces._
import memory._
import org.scalatest.{FlatSpec, Matchers}


class spmvMainIO(implicit val p: Parameters) extends Module with CoreParams with CacheParams {
  ...
  def cloneType = new spmvMainIO().asInstanceOf[this.type]
}

class spmvMain(implicit p: Parameters) extends spmvMainIO {
...
//  val memModel = Module(new NastiMemSlave) // Model of DRAM to connect to Cache
  ...
  val spmv = Module(new spmvDF())

  //Put an arbiter infront of cache
  val CacheArbiter = Module(new MemArbiter(2))

  // Connect input signals to cache
  CacheArbiter.io.cpu.MemReq(0) <> spmv.io.MemReq
  spmv.io.MemResp <> CacheArbiter.io.cpu.MemResp(0)
...
class spmvTest01[T <: spmvMainIO](c: T) extends PeekPokeTester(c) {
...
//  }
  ...
  poke(c.io.in.bits.data("field0").data, 0x00001fd1ff0) //
  poke(c.io.in.bits.data("field0").predicate, true.B)
  poke(c.io.in.bits.data("field1").data, 0x00001fd540) //
  poke(c.io.in.bits.data("field1").predicate, true.B)
  poke(c.io.in.bits.data("field2").data, 0x00001fd6e8) //
  poke(c.io.in.bits.data("field2").predicate, true.B)
  poke(c.io.in.bits.data("field3").data, 0x00001fd75c8) //
  poke(c.io.in.bits.data("field3").predicate, true.B)
  poke(c.io.in.bits.data("field4").data, 0x00001fd8538) //
...
class spmvTester1 extends FlatSpec with Matchers {
  implicit val p = config.Parameters.root((new MiniConfig).toInstance)
  val testParams = p.alterPartial({
    case XLEN => 32
    case TRACE => true
  })
  it should "Check that spmv works correctly." in {
    // iotester flags:
    ...
    chisel3.iotesters.Driver.execute(
      Array(
        ...
        "-tn", "spmvMain",
        "-tbn", "verilator",
        "-td", "test_run_dir/spmv",
        "-tts", "0001"),
      () => new spmvMain()(testParams)) {
      c => new spmvTest01(c)
    } should be(true)
  }
}

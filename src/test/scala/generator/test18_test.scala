...
class test18Main(implicit p: Parameters) extends test18MainIO {
  ...
  CacheArbiter.io.cpu.MemReq(0) <> test18.io.MemReq
  test18.io.MemResp <> CacheArbiter.io.cpu.MemResp(0)
...
class test18Test01[T <: test18MainIO](c: T) extends PeekPokeTester(c) {
...
class test18Tester1 extends FlatSpec with Matchers {
  implicit val p = config.Parameters.root((new MiniConfig).toInstance)
  it should "Check that test18 works correctly." in {
    // iotester flags:

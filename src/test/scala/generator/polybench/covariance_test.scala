...
class covarianceMainDirect(implicit p: Parameters) extends covarianceMainIO{
  ...
  val covariance = Module(new test13DF())


  cache.io.cpu.req <> covariance.io.MemReq
  covariance.io.MemResp<> cache.io.cpu.resp
...
class CovarianceTest01[T <: covarianceMainDirect](c: T) extends PeekPokeTester(c) {

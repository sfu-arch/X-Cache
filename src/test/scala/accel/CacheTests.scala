...
class CacheTester(cache: => Cache)(implicit val p: config.Parameters) extends BasicTester with CacheParams {
  ...
  val gold_resp_queue = Module(new Queue(new MemResp, 32))
  val gold_resp       = gold_resp_queue.io.deq
  ...
  switch(state) {
    ...
    is(sWait) {
      timeout := timeout + 1.U
      assert(timeout < 200.U)
      when(dut.io.cpu.resp.valid && gold_resp.valid) {
        when(!mask.orR) {
          when(dut.io.cpu.resp.bits.data =/= gold_resp.bits.data) {
            printf("ERROR resp.bits.data: %x != %x\n", dut.io.cpu.resp.bits.data, gold_resp.bits.data)
          }

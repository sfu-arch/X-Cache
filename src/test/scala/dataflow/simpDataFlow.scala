...
// Tester.
class simpleDFTester(df: simpleDataFlow)(implicit p: config.Parameters) extends PeekPokeTester(df)  {
...
class simpleDataFTests extends  FlatSpec with Matchers {
   implicit val p = config.Parameters.root((new MiniConfig).toInstance)
  it should "Dataflow tester" in {
     chisel3.iotesters.Driver(() => new simpleDataFlow()(p)) { c =>
       new simpleDFTester(c)
     } should be(true)
   }
 }

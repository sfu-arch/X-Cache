...
class Device {
   ...
   public:
    ...
    void Launch(vector<int32_t> &arguments) {
        ...
        dpi_->WriteReg(address + (cnt * 4), this->MemGetPhyAddr(a_));  // ptr(0)
        cnt++;
        dpi_->WriteReg(address + (cnt * 4), this->MemGetPhyAddr(b_));  // ptr(1)
...
int main(int argc, const char **argv) {
    ...
    vector<int32_t> arguments = {1, 2, 64, 4, 4096, 5, 128, 6, 128, 128, 8, 4, 16384};

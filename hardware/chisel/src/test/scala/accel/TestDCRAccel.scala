 ...
 * @tparam T
 */
abstract class DandelionTestDCRAccel[T <: memGenModule](implicit val p: Parameters) extends MultiIOModule with HasAccelShellParams
...
object DandelionTestDCRAccel {
   ...
   * @return
   */
  def apply(testName: String)(implicit p: Parameters): memGenModule = {
    testName match {
      case "memGenAccel" => new memGenAccel()

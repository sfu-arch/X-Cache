...
import dandelion.shell.{DandelionCacheShell, VTAShell}
...
//  val m_axi_gmem = IO(new XilinxAXIMaster(mp))
  ...
  val shell = withClockAndReset(clock = ap_clk, reset = ~ap_rst_n) {
    Module(new DandelionCacheShell(accelModule)(numPtrs, numVals, numRets, numEvents, numCtrls))
  }

  shell.io.mem <> DontCare

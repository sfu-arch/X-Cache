...
class test14DF(PtrsIn: Seq[Int] = List(32), ValsIn: Seq[Int] = List(32), Returns: Seq[Int] = List())
  ...
  val MemCtrl = Module(new CacheMemoryEngine(ID = 0, NumRead = 1, NumWrite = 2))
  ...
  val st_17 = Module(new UnTypStoreCache(NumPredOps = 0, NumSuccOps = 1, ID = 17, RouteID = 2))
  ...
  MemCtrl.io.wr.mem(0).MemReq <> st_6.io.MemReq

  st_6.io.MemResp <> MemCtrl.io.wr.mem(0).MemResp

  MemCtrl.io.rd.mem(0).MemReq <> ld_14.io.MemReq

  ld_14.io.MemResp <> MemCtrl.io.rd.mem(0).MemResp

  MemCtrl.io.wr.mem(1).MemReq <> st_17.io.MemReq

  st_17.io.MemResp <> MemCtrl.io.wr.mem(1).MemResp
